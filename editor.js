...
function renderBlockEl(block, i) {
  ...
  wrap.innerHTML = `<span class="rb-drag-handle" title="Drag to reorder">⠿</span>` + renderBlockContent(block);
  wrap.addEventListener('click', (e) => {
    if (e.target.closest('.rb-drag-handle')) return;
...
function renderBlockInPlace(id) {
  ...
  if (!el || !block) return;
  el.innerHTML = `<span class="rb-drag-handle" title="Drag to reorder">⠿</span>` + renderBlockContent(block);
...
// ── Canvas drag-to-reorder (via .rb-drag-handle) ─────────────────
let canvasDragEl = null, canvasPlaceholder = null, canvasDragOffsetY = 0;

function initCanvasDrag() {
  const paper = document.getElementById('resumePaper');

  paper.addEventListener('pointerdown', (e) => {
    const handle = e.target.closest('.rb-drag-handle');
    const block  = handle && handle.closest('.resume-block');
    ...
    canvasDragOffsetY = e.clientY - rect.top;

    canvasPlaceholder = document.createElement('div');
    canvasPlaceholder.className = 'rb-drop-placeholder';
    canvasPlaceholder.style.height = rect.height + 'px';
    block.after(canvasPlaceholder);
...
function onCanvasPointerMove(e) {
  if (!canvasDragEl) return;
  canvasDragEl.style.top = (e.clientY - canvasDragOffsetY) + 'px';

  // Find the col-track (or the flat paper for single-col) that the cursor is over
  const paper = document.getElementById('resumePaper');
  const siblings = Array.from(paper.querySelectorAll('.resume-block')).filter(b => b !== canvasDragEl);
  ...
  for (const sib of siblings) {
    ...
    if (midY < r.top + r.height / 2) {
      if (canvasPlaceholder.nextSibling !== sib) sib.before(canvasPlaceholder);
      inserted = true;
      break;
    }
  }
  if (!inserted && siblings.length) {
    ...
    if (canvasPlaceholder !== last.nextSibling) last.after(canvasPlaceholder);
  }
}
...
function onCanvasPointerUp() {
  ...
  canvasPlaceholder.replaceWith(canvasDragEl);
  canvasDragEl.classList.remove('rb-dragging');
  canvasDragEl.style.cssText = '';

  // Rebuild blocks order from the DOM
  const paper = document.getElementById('resumePaper');
  const orderedIds = Array.from(paper.querySelectorAll('.resume-block')).map(el => el.dataset.id);
...
// ── Toolbar ────────────────────────────────────────────────────
let guidesOn = false, darkCanvasOn = false;

function toggleGuides() {
  guidesOn = !guidesOn;
  document.getElementById('resumePaper').classList.toggle('show-guides', guidesOn);
  document.getElementById('showGuidesBtn').classList.toggle('active', guidesOn);
}
function toggleDarkPaper() {
  darkCanvasOn = !darkCanvasOn;
  document.getElementById('canvasWrap').classList.toggle('dark-canvas', darkCanvasOn);
  document.getElementById('darkPaperBtn').classList.toggle('active', darkCanvasOn);
}
